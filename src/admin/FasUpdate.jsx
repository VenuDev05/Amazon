...
const FasUpdate = () => {
    ...
    useEffect(() => {
        fetch(`${API_URL}/fasDataReceive`).then((res) => res.json()).then((data) => setProductItems(data), [])
    })
    const deleteItems = (id) => {
        fetch(`${API_URL}/fasDataDelete/${id}`, {
            method: "DELETE"
    ...
    return (
        <div id='update-whole'>
            <li>
                <Link to='/fasUpload' id='updateBtn'>⬅️Upload</Link>
            </li>
            <table border='1'>
                <thead>
                    <tr>
